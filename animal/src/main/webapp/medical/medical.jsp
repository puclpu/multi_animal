...
<body>
	...
	<div id="medicalSection">
		<div class="location-map-wrap">
			...
			<script>
			sessionStorage.setItem("myLocationArea1", '')
			sessionStorage.setItem("myLocationArea2", '')
			...
			naver.maps.Event.addListener(map, 'click', function(e) {
			    ...
			    naver.maps.Service.reverseGeocode({
					...
					sessionStorage.removeItem("myLocationArea1")
					sessionStorage.removeItem("myLocationArea2")
					...
					naver.maps.Service
							.reverseGeocode(
									...
									function(status, response) {
										...
										var loadArea1 = sessionStorage.setItem("myLocationArea1", result.results[0].region.area1.alias)
										var loadArea2 = sessionStorage.setItem("myLocationArea2", result.results[0].region.area2.name)
										
										$.ajax({
											url : 'medicalAddrResult',
											data : {
												medicalLocationArea1:result.results[0].region.area1.alias,
												medicalLocationArea2:result.results[0].region.area2.name
											},
											success : function(x) {
												$('#medicalSearchResult').html(x)
